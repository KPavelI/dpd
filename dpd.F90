...
integer ierr
integer*4 steps1,steps2,steps3,qstp,rststp,sqfstp,vtkstp,nstp,chemstp,stepsdef,ndef
...
do nstp=1,steps2
    ...
    ! output restart file
    if (mod(nstp,rststp)==0.and.rst==1 .and. nstp>80000000) call writerst()
end do
...
do i = 1, natms
    ...
    if (kindp(i-shft,1).ne.4) then
    if (rxb + (ncol-1)*dlx > dlxa-shwi .or. rxb + (ncol-1)*dlx < shwi) then
        vxb=(-1)*vxb
        vwxb=(-1)*vwxb
        rxb  = rx(i-shft,1) + dt*vxb
    end if
    if (ryb + (nrow-1)*dly > dlya-shwi .or. ryb + (nrow-1)*dly < shwi) then
        vyb=(-1)*vyb
        vwyb=(-1)*vwyb
        ryb  = ry(i-shft,1) + dt*vyb
    end if
    if (rzb > dlza-shwi .or. rzb < shwi) then
       vzb = (-1)*vzb
       vwzb = (-1)*vwzb
       rzb  = rz(i-shft,1) + dt*vzb
    end if
    if ( rxb + (ncol-1)*dlx > dlxa .or. rxb + (ncol-1)*dlx < 0 .or. ryb + (nrow-1)*dly > dlya .or. ryb + (nrow-1)*dly < 0 .or. rzb > dlza .or. rzb < 0) then
       write(*,*) "rxb = ", rxb
       write(*,*) "ryb = ", ryb
       write(*,*) "rzb = ", rzb
       write(*,*) " fx, fy, fz "
       write(*,*) fx(i-shft), fy(i-shft), fz(i-shft)
    end if
    end if
...
real*4 rho,shwi
integer*4 rst,vtk,sqf,qstp,vtkstp,sqfstp,rststp,ndef
real*4 tai,tau
...
do k=1,snum
    ...
    elseif (fnames(k)(1:trimtext(fnames(k)))=='sphere.gen') then
        ...
        do i = 1, nat
            ...
            !if (i==1) write(*,*) "about reading:   ", i, rxt(i),ryt(i),rzt(i),kindptc
            do j=1,10
    ...
    ! read bond structure
        ...
        do i=1,nmol
            ...
            if (place(k)==1) then
                ...
                rndposy=(uni())*dlya
                rndposz=(uni())*dlza
            ! coordinates from mol-file
			else if (place(k)==3) then
                rndposx=0
                rndposy=0
                rndposz=0
            ! random beads position variant
            else
                call rndplace( nat,rxt,ryt,rzt,cn2,dlxa,dlya,dlza )
                rndposx=0
                rndposy=0
                rndposz=0
            end if
...
subroutine rndplace( nat,rxt,ryt,rzt,cn,dlxa,dlya,dlza )
! #################################################################
! #                                                               #
! #    subroutine 36:                                             #
